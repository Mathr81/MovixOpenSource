...
async function loadInjectedJavaScriptBuilder() {
  ...
  const require = id => {
    ...
    if (id === './popup-redirect') return { buildPopupRedirectScript: () => 'POPUP_REDIRECT' };
    if (id === './site-storage-sync') {
      return {
        buildStorageRestoreScript: () => 'STORAGE_RESTORE',
        buildStorageCaptureScript: () => 'STORAGE_CAPTURE',
      };
    }
